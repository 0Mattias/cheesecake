...
- Chunk data written to the elytra pathfinder is bounded by the height its context was created with. Upstream bounds the packing by the world's section count and block updates by a literal 384, which agree with the context everywhere except the Nether with `elytraAllowAboveRoof` off: there the context is 128 tall and the dimension has sixteen sections, so every packed chunk writes twice what was allocated for it. The native heap is corrupted, and the client dies later and elsewhere -- inside the pathfinder, inside `malloc`, or with `raytrace whiffed` and a status of 137 that reads like being killed for running out of memory.
...
- The elytra process gives up after thirty seconds on the ground. Upstream can wait for ever in either of the states that precede flight: one asks for a walking path to a ledge it may have no route to and pauses the path executor while it waits, so the failure returns as `NEXT_CALC_FAILED` rather than the event its give-up is watching for, and the other presses jump only while already falling, which never becomes true standing still. Either way the bot stands there until something else stops it.
