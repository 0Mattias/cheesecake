...
- The Nether pathfinder is babbaj's nether-pathfinder ported to Java, under `cheesecake.process.elytra.pathfinder`, rather than the native library upstream bundles for each platform. Its terrain generator and raytracer reproduce the native library's recorded answers to the bit and run at the same speed; there is nothing to load, so no system is unsupported, no `Unsafe`, and no memory that can be freed under a thread still reading it. The port was written for [babbaj/nether-pathfinder#31](https://github.com/babbaj/nether-pathfinder/issues/31), which asks for one, and is offered to Baritone.
- Rays of zero length are answered without asking the pathfinder, as a point is visible from itself; rays with a coordinate that is not a finite number are answered as not visible; and a ray that ends exactly on a voxel boundary is asked about with its end moved a millionth of a block towards its start, so that a ray aimed at a path node on the corner of a block is answered by the voxel it crosses last rather than by rounding. Upstream hands all three to the library as they are. With the native library the first two ended the game -- it prints `raytrace whiffed` and calls `exit(696969)`, which leaves status 137 and reads as a kill -- and an end on a boundary could do the same from some directions, filed as [babbaj/nether-pathfinder#23](https://github.com/babbaj/nether-pathfinder/issues/23); the port answers all three.
- The landing spot's visibility check runs on a worker thread holding the pathfinder's read lock, and the cached chunk the solver's block lookups share is one immutable pair rather than three fields, since two solvers overlap for a moment whenever a flight is re-planned. Upstream runs the check on the game thread with no lock, in the one path-finishing step that raytraces, while the write executor is still packing the chunks a flying player loads; with the native library that could traverse a chunk being freed, and a path calculation with a region cache could insert chunks under a reader, filed as [babbaj/nether-pathfinder#25](https://github.com/babbaj/nether-pathfinder/issues/25). The port's chunk table takes all of that from any thread, and the lock is kept because it is what the flights were tested with. A path calculation that does not generate terrain runs under the read lock, as upstream runs it; taking the write lock for those was tried and grounded flights, because the game thread cannot steer while a writer holds the lock.
- The elytra process keeps its pathfinder context between flights and rebuilds it only when the world or the settings it was built for change, so the chunks packed for one flight are there for the next. Upstream frees it after every flight and builds a new one, which with the native library could hang the first path calculation for good -- its `ParallelExecutor` started each worker before the flag that stops it was constructed, filed as [babbaj/nether-pathfinder#24](https://github.com/babbaj/nether-pathfinder/issues/24). A calculation that has not answered in thirty seconds is still reported and its context rebuilt for the next `#elytra`, as a safety net.
- The elytra landing search looks at the surface when it finds nowhere to land below, and a search that has run out is kept until the player has moved a chunk. Upstream's underground search is seeded at the destination's nominal height, and when that is inside the ground it queues nothing, ends at once, and is rebuilt and announced again every tick -- which left a player low over hills, under a platform in the sky, circling with nowhere to go.
...
- `#find` searches the cache around the player's x and z. Upstream passes the player's y as the z.
- Spawner avoidance looks the cache up under `spawner`, the block's name since 1.13. Upstream asks for `mob_spawner` and never finds one.

### Verification

Every push is compiled against Minecraft 26.2, the unit tests are run, the jar is checked for packaging, and a client is launched under a virtual display to confirm that every mixin applies. A second client then creates a survival world from a fixed seed and plays through a series of scenarios, building what each needs with server commands: it walks a fixed distance with the goal drawn as the box and then as the beacon beam and photographs the beam; drives a trip over the control socket; explores towards chunks it has not cached; climbs free-hanging, twisting and weeping vines; runs `#mine` twice and checks that it stops at the requested count of what the blocks drop; builds a box from a `#sel` selection; follows an animal; farms a ripe field; finds a block with `#goto`; mines a tunnel that the backfill must close again; swaps a pickaxe into the hotbar only while the pauser holds the path; prices a step into water, onto lava and onto a waterlogged stair with and without Frost Walker boots; walks under free look with the camera turned away; and flies with an elytra above the build limit in the Overworld, through Nether terrain below the roof, and over the roof, landing each time. Terrain the fixed seed does not provide is not covered, and reports are welcome.
