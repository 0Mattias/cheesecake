...
- **in-world pathing test** launches the client with `CHEESECAKE_AUTO_TEST=true`. The client creates a survival world from a fixed seed, walks a fixed distance with the pathfinder, and exits when it arrives. `CheesecakeAutoTest` is the routine; run it locally the same way and it leaves the world under `run/saves`.
