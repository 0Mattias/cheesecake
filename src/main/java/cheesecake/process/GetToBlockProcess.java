...
import cheesecake.api.process.IGetToBlockProcess;
...
public final class GetToBlockProcess extends CheesecakeProcessHelper implements IGetToBlockProcess {
    ...
    public GetToBlockProcess(Cheesecake cheesecake) {
        super(cheesecake);
    }
    ...
    public synchronized PathingCommand onTick(boolean calcFailed, boolean isSafeToCancel) {
        ...
        if (knownLocations.isEmpty()) {
            if (Cheesecake.settings().exploreForBlocks.value && !calcFailed) {
        ...
        if (calcFailed) {
            if (Cheesecake.settings().blacklistClosestOnFailure.value) {
        ...
        if (mineGoalUpdateInterval != 0 && tickCount++ % mineGoalUpdateInterval == 0) { // big brain
            ...
            Cheesecake.getExecutor().execute(() -> rescan(current, context));
        }
        if (goal.isInGoal(ctx.playerFeet()) && goal.isInGoal(cheesecake.getPathingBehavior().pathStart()) && isSafeToCancel) {
            // we're there
    ...
    public class GetToBlockCalculationContext extends CalculationContext {
        ...
        public GetToBlockCalculationContext(boolean forUseOnAnotherThread) {
            super(GetToBlockProcess.super.cheesecake, forUseOnAnotherThread);
        }
    ...
    private boolean walkIntoInsteadOfAdjacent(Block block) {
        if (!Cheesecake.settings().enterPortal.value) {
            return false;
        }
    ...
    private boolean rightClickOnArrival(Block block) {
        if (!Cheesecake.settings().rightClickContainerOnArrival.value) {
            return false;
        }
