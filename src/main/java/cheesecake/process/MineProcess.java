...
import cheesecake.api.process.IMineProcess;
 ...
 * @author leijurv
 */
public final class MineProcess extends CheesecakeProcessHelper implements IMineProcess {
    ...
    public MineProcess(Cheesecake cheesecake) {
        super(cheesecake);
    }
    ...
    public PathingCommand onTick(boolean calcFailed, boolean isSafeToCancel) {
        ...
        if (calcFailed) {
            if (!knownOreLocations.isEmpty() && Cheesecake.settings().blacklistClosestOnFailure.value) {
        ...
        updateLoucaSystem();
        int mineGoalUpdateInterval = Cheesecake.settings().mineGoalUpdateInterval.value;
        List<BlockPos> curr = new ArrayList<>(knownOreLocations);
        if (mineGoalUpdateInterval != 0 && tickCount++ % mineGoalUpdateInterval == 0) { // big brain
            CalculationContext context = new CalculationContext(cheesecake, true);
            Cheesecake.getExecutor().execute(() -> rescan(curr, context));
        }
        if (Cheesecake.settings().legitMine.value) {
            if (!addNearby()) {
                cancel();
                return null;
            }
        ...
        if (shaft.isPresent() && ctx.player().isOnGround()) {
            ...
            if (!MovementHelper.avoidBreaking(cheesecake.bsi, pos.getX(), pos.getY(), pos.getZ(), state)) {
                ...
                if (rot.isPresent() && isSafeToCancel) {
                    cheesecake.getLookBehavior().updateTarget(rot.get(), true);
    ...
    private void updateLoucaSystem() {
        ...
        ctx.getSelectedBlock().ifPresent(pos -> {
            if (knownOreLocations.contains(pos)) {
                copy.put(pos, System.currentTimeMillis() + Cheesecake.settings().mineDropLoiterDurationMSThanksLouca.value);
            }
        });
    ...
    private PathingCommand updateGoal() {
        ...
        boolean legit = Cheesecake.settings().legitMine.value;
        List<BlockPos> locs = knownOreLocations;
        if (!locs.isEmpty()) {
            CalculationContext context = new CalculationContext(cheesecake);
            List<BlockPos> locs2 = prune(context, new ArrayList<>(locs), filter, Cheesecake.settings().mineMaxOreLocationsCount.value, blacklist, droppedItemsScan());
        ...
        if (!legit && !Cheesecake.settings().exploreForBlocks.value) {
            return null;
        }
        // only when we should explore for blocks or are in legit mode we do this
        int y = Cheesecake.settings().legitMineYLevel.value;
        if (branchPoint == null) {
            /*if (!cheesecake.getPathingBehavior().isPathing() && playerFeet().y == y) {
    ...
    private void rescan(List<BlockPos> already, CalculationContext context) {
        ...
        List<BlockPos> locs = searchWorld(context, filter, Cheesecake.settings().mineMaxOreLocationsCount.value, already, blacklist, dropped);
        locs.addAll(dropped);
        if (locs.isEmpty() && !Cheesecake.settings().exploreForBlocks.value) {
    ...
    private boolean internalMiningGoal(BlockPos pos, CalculationContext context, List<BlockPos> locs) {
        ...
        if (Cheesecake.settings().internalMiningAirException.value && state.getBlock() instanceof AirBlock) {
            return true;
        }
    ...
    public List<BlockPos> droppedItemsScan() {
        if (!Cheesecake.settings().mineScanDroppedItems.value) {
    ...
    public static List<BlockPos> searchWorld(CalculationContext ctx, BlockOptionalMetaLookup filter, int max, List<BlockPos> alreadyKnown, List<BlockPos> blacklist, List<BlockPos> dropped) {
        ...
        for (BlockOptionalMeta bom : filter.blocks()) {
            ...
            if (CachedChunk.BLOCKS_TO_KEEP_TRACK_OF.contains(block)) {
                BetterBlockPos pf = ctx.cheesecake.getPlayerContext().playerFeet();

                // maxRegionDistanceSq 2 means adjacent directly or adjacent diagonally; nothing further than that
                locs.addAll(ctx.worldData.getCachedWorld().getLocationsOf(
                        BlockUtils.blockToString(block),
                        Cheesecake.settings().maxCachedWorldScanCount.value,
                        pf.x,
                        pf.z,
                        2
                ));
        ...
        if (!untracked.isEmpty() || (Cheesecake.settings().extendCacheOnThreshold.value && locs.size() < max)) {
            locs.addAll(CheesecakeAPI.getProvider().getWorldScanner().scanChunkRadius(
                    ctx.getCheesecake().getPlayerContext(),
                    filter,
                    max,
                    10,
                    32
    ...
    private boolean addNearby() {
        ...
        knownOreLocations = prune(new CalculationContext(cheesecake), knownOreLocations, filter, Cheesecake.settings().mineMaxOreLocationsCount.value, blacklist, dropped);
        return true;
    }
    ...
    private static List<BlockPos> prune(CalculationContext ctx, List<BlockPos> locs2, BlockOptionalMetaLookup filter, int max, List<BlockPos> blacklist, List<BlockPos> dropped) {
        ...
        List<BlockPos> locs = locs2
                ...
                .filter(pos -> {
                    if (Cheesecake.settings().allowOnlyExposedOres.value) {
    ...
    public static boolean isNextToAir(CalculationContext ctx, BlockPos pos) {
        int radius = Cheesecake.settings().allowOnlyExposedOresDistance.value;
    ...
    public void mine(int quantity, BlockOptionalMetaLookup filter) {
        ...
        if (filter != null) {
            rescan(new ArrayList<>(), new CalculationContext(cheesecake));
        }
    }
    ...
    private BlockOptionalMetaLookup filterFilter() {
        ...
        if (!Cheesecake.settings().allowBreak.value) {
            BlockOptionalMetaLookup f = new BlockOptionalMetaLookup(this.filter.blocks()
                    .stream()
                    .filter(e -> Cheesecake.settings().allowBreakAnyway.value.contains(e.getBlock()))
