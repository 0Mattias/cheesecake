...
import cheesecake.api.behavior.look.IAimProcessor;
import cheesecake.api.behavior.look.ITickableAimProcessor;
...
import static cheesecake.utils.CheesecakeMath.fastCeil;
import static cheesecake.utils.CheesecakeMath.fastFloor;
...
public final class ElytraBehavior implements Helper {
    ...
    public ElytraBehavior(Cheesecake cheesecake, ElytraProcess process, NetherPathfinderContext npf, BlockPos destination, boolean appendDestination) {
        ...
        if (ctx.world().getRegistryKey() == World.NETHER) {
            this.pathFinder = Cheesecake.settings().elytraAllowAboveRoof.value && Cheesecake.settings().elytraAllowAboveBuildLimit.value
                    ? new BuildLimitPathFinder(ctx, npfContext)
                    : npfContext;
        } else {
            this.pathFinder = Cheesecake.settings().elytraAllowAboveBuildLimit.value ? new BuildLimitPathFinder(ctx, npfContext) : npfContext;
        }
    }
    ...
    public final class PathManager {
            ...
            return this.path0(from, destinationFixed(), UnaryOperator.identity())
                    .thenRun(() -> {
                        ...
                        if (this.completePath) {
                            logVerbose(String.format("Computed path (%.1f blocks in %.4f seconds)", distance,
                                    (System.nanoTime() - start) / 1e9d));
                        } else {
                            logVerbose(String.format("Computed segment (Next %.1f blocks in %.4f seconds)", distance,
                                    (System.nanoTime() - start) / 1e9d));
                        }
                    })
        ...
        public CompletableFuture<Void> pathRecalcSegment(final OptionalInt upToIncl) {
            ...
            final List<BetterBlockPos> after = upToIncl.isPresent()
                    ? this.path.subList(upToIncl.getAsInt() + 1, this.path.size())
                    : Collections.emptyList();
            final boolean complete = this.completePath;

            return this
                    .path0(ctx.playerFeet(),
                            upToIncl.isPresent() ? fixDestination(this.path.get(upToIncl.getAsInt())) : destinationFixed(),
                            segment -> segment.append(after.stream(),
                                    complete || (segment.isFinished() && !upToIncl.isPresent())))
                    .whenComplete((result, ex) -> {
                        this.recalculating = false;
                        if (ex != null) {
                            final Throwable cause = ex.getCause();
        ...
        private void pathfindAroundObstacles() {
            ...
            for (int i = rangeStartIncl; i < rangeEndExcl - 1; i++) {
                ...
                if (!ElytraBehavior.this.clearView(this.path.getVec(i), this.path.getVec(i + 1), false)) {
                    ...
                    // if the end of render distance is closer to goal, then that's fine, otherwise
                    // we'd be "digging our hole deeper" and making an already bad backtrack worse
                    ...
                    if (this.path.get(rangeEndExcl - 1).distanceSq(dest) < ctx.playerFeet().distanceSq(dest)) {
                        ...
                        rejoinMainPathAt = OptionalInt.empty(); // large backtrack detected. ignore render distance,
                                                                // rejoin later on
                    }

                    final BetterBlockPos blockage = this.path.get(i);
                    final double distance = ctx.playerFeet()
                            .distanceTo(this.path.get(rejoinMainPathAt.orElse(path.size() - 1)));
            ...
            if (!canSeeAny && rangeStartIncl < rangeEndExcl - 2 && process.state != ElytraProcess.State.GET_TO_JUMP) {
                this.pathRecalcSegment(OptionalInt.of(rangeEndExcl - 1))
                        .thenRun(() -> logVerbose("Recalculated segment since no path points were visible"));
            }
        }
        ...
        private void attemptNextSegment() {
            ...
            final BetterBlockPos lastPos = this.path.get(last);
            // `ctx.world().isPosLoaded` cannot be used here as it returns false if the y-value is beyond the build limits.
            if (!this.completePath && ctx.world().getChunkManager().isChunkLoaded(lastPos.x >> 4, lastPos.z >> 4)) {
    ...
    public void onRenderPass(RenderEvent event) {
        ...
        final Settings settings = Cheesecake.settings();
        if (this.visiblePath != null) {
            PathRenderer.drawPath(event.getModelViewStack(), this.visiblePath, 0, Color.RED, false, 0, 0, 0.0D);
        }
        if (this.aimPos != null) {
            PathRenderer.drawGoal(event.getModelViewStack(), ctx, new GoalBlock(this.aimPos), event.getPartialTicks(),
                    Color.GREEN);
        }
    ...
    public void onReceivePacket(PacketEvent event) {
        if (event.getPacket() instanceof PlayerPositionLookS2CPacket) {
            ctx.minecraft().execute(() -> {
                this.remainingSetBackTicks = Cheesecake.settings().elytraFireworkSetbackUseDelay.value;
            });
        }
    }

    public void pathTo() {
        if (!Cheesecake.settings().elytraAutoJump.value || ctx.player().isGliding()) {
    ...
    public void onTick() {
        ...
        if ((now - this.timeLastCacheCull) / 1000 > Cheesecake.settings().elytraTimeBetweenCacheCullSecs.value) {
            npfContext.queueCacheCulling(ctx.player().getChunkPos().x, ctx.player().getChunkPos().z, Cheesecake.settings().elytraCacheCullDistance.value);
    ...
    /**
     * Called by {@link cheesecake.process.ElytraProcess#onTick(boolean, boolean)}
    ...
    public void tick() {
        ...
        cheesecake.getLookBehavior().updateTarget(solution.rotation, false);
        ...
        for (int relaxation = 0; relaxation < 3; relaxation++) { // try for a strict solution first, then relax more and
            ...
            for (int i = Math.min(playerNear + 20, path.size() - 1); i >= minStep; i--) {
                ...
                for (final Pair<Vec3d, Integer> candidate : candidates) {
                    ...
                    final double minAvoidance = Cheesecake.settings().elytraMinimumAvoidance.value;
    ...
    private void tickUseFireworks(final Vec3d start, final Vec3d goingTo, final boolean isBoosted,
        ...
        final boolean useOnDescend = !Cheesecake.settings().elytraConserveFireworks.value
                || ctx.player().getY() < goingTo.y + 5;
        final double currentSpeed = new Vec3d(
                ctx.player().getVelocity().getX(),
                // ignore y component if we are BOTH below where we want to be AND descending
                ctx.player().getY() < goingTo.y ? Math.max(0, ctx.player().getVelocity().getY())
                        : ctx.player().getVelocity().getY(),
                ctx.player().getVelocity().getZ()).lengthSquared();

        final double elytraFireworkSpeed = Cheesecake.settings().elytraFireworkSpeed.value;
        if (this.remainingFireworkTicks <= 0 && (forceUseFirework || (!isBoosted
                && useOnDescend
                && (ctx.player().getY() < goingTo.y - 5
                        || start.distanceTo(new Vec3d(goingTo.x + 0.5, ctx.player().getY(), goingTo.z + 0.5)) > 5) // UGH!!!!!!!
            ...
            if (!cheesecake.getInventoryBehavior().throwaway(true, ElytraBehavior::isBoostingFireworks) &&
                    !cheesecake.getInventoryBehavior().throwaway(true, ElytraBehavior::isFireworks)) {
    ...
    private final class SolverContext {
        ...
        public SolverContext(boolean async) {
            ...
            ITickableAimProcessor aim = ElytraBehavior.this.cheesecake.getLookBehavior().getAimProcessor().fork();
            if (async) {
    ...
    public static boolean isFireworks(final ItemStack itemStack) {
        ...
        net.minecraft.component.type.FireworksComponent fireworks = itemStack
                .get(net.minecraft.component.DataComponentTypes.FIREWORKS);
    ...
    private boolean isHitboxClear(final SolverContext context, final Vec3d dest, final Double growAmount) {
        ...
        if (Cheesecake.settings().elytraRenderHitboxRaytraces.value) {
            boolean clear = true;
            for (int i = 0; i < 8; i++) {
    ...
    public boolean clearView(Vec3d start, Vec3d dest, boolean ignoreLava) {
        ...
        if (!ignoreLava) {
            ...
            clear = ctx.world().raycast(new RaycastContext(start, dest, RaycastContext.ShapeType.COLLIDER,
                    RaycastContext.FluidHandling.NONE, ctx.player())).getType() == HitResult.Type.MISS;
        }

        if (Cheesecake.settings().elytraRenderRaytraces.value) {
    ...
    private static FloatArrayList pitchesToSolveFor(final float goodPitch, final boolean desperate) {
        final float minPitch = desperate ? -90
                : Math.max(goodPitch - Cheesecake.settings().elytraPitchRange.value, -89);
        final float maxPitch = desperate ? 90 : Math.min(goodPitch + Cheesecake.settings().elytraPitchRange.value, 89);
    ...
    private Pair<Float, Boolean> solvePitch(final SolverContext context, final Vec3d goal, final int relaxation) {
        ...
        final int ticks = desperate ? 3
                : context.boost.isBoosted() ? Math.max(5, context.boost.getGuaranteedBoostTicks())
                        : Cheesecake.settings().elytraSimulationTicks.value;
...
//             final double cz = hitbox.minZ + (hitbox.maxZ - hitbox.minZ) * 0.5D;
            ...
            // stretch is the directional swept volume (upstream #5049): unlike expand it handles
            // negative motion correctly, and unlike offset it covers the whole travelled volume.
    ...
    private void tickInventoryTransactions() {
        if (invTickCountdown <= 0) {
            ...
            if (r != null) {
                r.run();
                invTickCountdown = Cheesecake.settings().ticksBetweenInventoryMoves.value;
            }
        }
        if (invTickCountdown > 0)
            invTickCountdown--;
    ...
    private int findGoodElytra() {
        ...
        for (int i = 0; i < invy.size(); i++) {
            ...
            if (slot.getItem() == Items.ELYTRA && (slot.getMaxDamage()
                    - slot.getDamage()) > Cheesecake.settings().elytraMinimumDurability.value) {
                return i;
            }
        }
        return -1;
    ...
    private void trySwapElytra() {
        if (!Cheesecake.settings().elytraAutoSwap.value || !invTransactionQueue.isEmpty()) {
            return;
        }
        ...
        if (chest.getItem() != Items.ELYTRA
                || chest.getMaxDamage()
                        - chest.getDamage() > Cheesecake.settings().elytraMinimumDurability.value) {
            return;
        }
    ...
    void logVerbose(String message) {
        if (Cheesecake.settings().elytraChatSpam.value) {
            logDebug(message);
        }
    }
    ...
    private boolean vanillaClear(Vec3d start, Vec3d end) {
        return ctx.world().raycast(new RaycastContext(start, end, RaycastContext.ShapeType.COLLIDER, RaycastContext.FluidHandling.NONE, ctx.player())).getType() == HitResult.Type.MISS;
    }

    public boolean raytrace(double startX, double startY, double startZ, double endX, double endY, double endZ) {
        final int maxHeight = npfContext.getMaxHeight() + ctx.world().getBottomY();
        final int minHeight = ctx.world().getBottomY();
        final boolean isOOB = startY >= maxHeight || endY >= maxHeight || startY < minHeight || endY < minHeight;
        if (isOOB) {
            return vanillaClear(new Vec3d(startX, startY, startZ), new Vec3d(endX, endY, endZ));
        }

        return npfContext.raytrace(startX, startY, startZ, endX, endY, endZ);
    }

    public boolean raytrace(Vec3d start, Vec3d end) {
        final int maxHeight = npfContext.getMaxHeight() + ctx.world().getBottomY();
        final int minHeight = ctx.world().getBottomY();
        final boolean isOOB = start.y >= maxHeight || end.y >= maxHeight || start.y < minHeight || end.y < minHeight;
        if (isOOB) {
            return vanillaClear(start, end);
        }
    ...
    public boolean raytrace(int count, double[] src, double[] dst, int visibility) {
        ...
        for (int i = 1; i < src.length; i += 3) {
            if (src[i] >= maxHeight || src[i] < minHeight || dst[i] >= maxHeight || dst[i] < minHeight) {
                isOOB = true;
                break;
            }
        }
        ...
        if (isOOB) {
            for (int i = 0; i < count; i++) {
                Vec3d start = new Vec3d(src[i * 3], src[i * 3 + 1], src[i * 3 + 2]);
                Vec3d end = new Vec3d(dst[i * 3], dst[i * 3 + 1], dst[i * 3 + 2]);
                if (!vanillaClear(start, end)) {
                    return false;
                }
            }
            return true;
    ...
    public boolean passable(int x, int y, int z) {
        if (y > ctx.world().getTopYInclusive() || y < ctx.world().getBottomY()) {
            return true;
        }
