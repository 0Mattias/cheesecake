 ...
 * nether pathfinder only the transitions up and down. Backport of upstream Baritone's
 * BuildLimitPathFinder.
 */
public class BuildLimitPathFinder implements IElytraPathFinder {
     ...
     * @return The path as a list of BetterBlockPos and a boolean indicating if the path is complete
     */
     ...
     * @param start       The start position
     * @param destination The destination
     * @return The path that transitions above build limit and a boolean indicating if a transition was found
     */
    ...
    /**
     * Attempts to find an open spot in the sky to transition down to a flight level the nether pathfinder can navigate at.
     *
     * @param start The start position
    ...
    public CompletableFuture<UnpackedSegment> pathFindAsync(BlockPos src, BlockPos dst) {
        ...
        if (srcAboveSupportedHeight && dstAboveSupportedHeight) {
            Pair<List<BetterBlockPos>, Boolean> path = generateDirectPath(new BetterBlockPos(src), new BetterBlockPos(dst), 0, maxDirectPathSize);
            return CompletableFuture.completedFuture(new UnpackedSegment(path.first().stream(), path.second()));
        }

        if (isLongDistance) {
            if (srcAboveSupportedHeight) {
                Pair<List<BetterBlockPos>, Boolean> directPath = generateDirectPath(new BetterBlockPos(src), new BetterBlockPos(dst), (int) maxDistance, maxDirectPathSize);
                ...
                Pair<List<BetterBlockPos>, Boolean> transition = generateTransitionUp(new BetterBlockPos(src), new BetterBlockPos(dst));
                List<BetterBlockPos> path = transition.first();
                boolean success = transition.second();

                if (success) {
                    Pair<List<BetterBlockPos>, Boolean> directPath = generateDirectPath(path.get(path.size() - 1), new BetterBlockPos(dst), (int) maxDistance, maxDirectPathSize);
            ...
            if (srcAboveSupportedHeight) {
                Pair<List<BetterBlockPos>, Boolean> transition = generateTransitionDown(new BetterBlockPos(src));
                List<BetterBlockPos> path = transition.first();
                boolean success = transition.second();
                ...
                return CompletableFuture.supplyAsync(() -> {
                    UnpackedSegment np = blockingPathFind(path.get(path.size() - 1), dst);
            ...
            if (dstAboveSupportedHeight) {
                ...
                if (success) {
                    Pair<List<BetterBlockPos>, Boolean> directPath = generateDirectPath(path.get(path.size() - 1), new BetterBlockPos(dst), 0, maxDirectPathSize);
                    path.addAll(directPath.first());
