...
import cheesecake.api.command.datatypes.RelativeFile;
...
import cheesecake.api.command.helpers.Paginator;
...
import static cheesecake.api.utils.SettingsUtil.*;

@SuppressWarnings({"unchecked", "rawtypes"})
public class SetCommand extends Command {

    public SetCommand(ICheesecake cheesecake) {
        super(cheesecake, "set", "setting", "settings");
    }
    ...
    public void execute(String label, IArgConsumer args) throws CommandException {
        ...
        if (Arrays.asList("s", "save").contains(arg)) {
            SettingsUtil.save(Cheesecake.settings());
        ...
        if (Arrays.asList("load", "ld").contains(arg)) {
            ...
            // reset to defaults
            SettingsUtil.modifiedSettings(Cheesecake.settings()).forEach(Settings.Setting::reset);
            // then load from disk
            SettingsUtil.readAndApply(Cheesecake.settings(), file);
        ...
        if (paginate) {
            ...
            List<? extends Settings.Setting> toPaginate = (viewModified
                    ? SettingsUtil.modifiedSettings(Cheesecake.settings())
                    : Cheesecake.settings().allSettings).stream()
            ...
            Paginator.paginate(
                    ...
                    () -> logDirect(
                            !search.isEmpty()
                                    ? String.format("All %ssettings containing the string '%s':",
                    ...
                    setting -> {
                        ...
                        String commandSuggestion = Cheesecake.settings().prefix.value
        ...
        Settings.Setting<?> setting = Cheesecake.settings().allSettings.stream()
        ...
        if (!doingSomething && !args.hasAny()) {
            ...
            if (resetting) {
                ...
                if (setting.getValueClass() != Boolean.class) {
                    throw new CommandInvalidTypeException(args.consumed(), "a toggleable setting",
                            "some other setting");
                }
                ...
                try {
                    SettingsUtil.parseAndApply(Cheesecake.settings(), arg, newValue);
                } catch (Throwable t) {
                    t.printStackTrace();
            ...
            if ((setting.getName().equals("chatControl") && !(Boolean) setting.value
                    && !Cheesecake.settings().chatControlAnyway.value) ||
                    setting.getName().equals("chatControlAnyway") && !(Boolean) setting.value
                            && !Cheesecake.settings().chatControl.value) {
                logDirect(
            ...
            if (args.hasExactlyOne()
                ...
                if (arg.equalsIgnoreCase("reset")) {
                    ...
                    return RelativeFile.tabComplete(args,
                            MinecraftClient.getInstance().runDirectory.toPath().resolve("cheesecake").toFile());
                }
                Settings.Setting setting = Cheesecake.settings().byLowerName.get(arg.toLowerCase(Locale.US));
                if (setting != null) {
    ...
    public List<String> getLongDesc() {
        return Arrays.asList(
                "Using the set command, you can manage all of Cheesecake's settings. Almost every aspect is controlled by these settings - go wild!",
                "",
                "Usage:",
                ...
                "> set load [filename] - Load settings from another file in your minecraft/cheesecake");
    }
}
