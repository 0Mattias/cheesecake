...
import cheesecake.api.command.datatypes.ForAxis;
import cheesecake.api.command.datatypes.ForBlockOptionalMeta;
import cheesecake.api.command.datatypes.ForDirection;
import cheesecake.api.command.datatypes.RelativeBlockPos;
...
import cheesecake.api.schematic.*;
import cheesecake.api.schematic.mask.shape.CylinderMask;
import cheesecake.api.schematic.mask.shape.SphereMask;
import cheesecake.api.selection.ISelection;
import cheesecake.api.selection.ISelectionManager;
...
public class SelCommand extends Command {
    ...
    private ISelectionManager manager = cheesecake.getSelectionManager();
    private BetterBlockPos pos1 = null;
    private ISchematic clipboard = null;
    private Vec3i clipboardOffset = null;

    public SelCommand(ICheesecake cheesecake) {
        super(cheesecake, "sel", "selection", "s");
        cheesecake.getGameEventHandler().registerEventListener(new AbstractGameEventListener() {
            @Override
            public void onRenderPass(RenderEvent event) {
                if (!Cheesecake.settings().renderSelectionCorners.value || pos1 == null) {
                    return;
                }
                Color color = Cheesecake.settings().colorSelectionPos1.value;
    ...
    public void execute(String label, IArgConsumer args) throws CommandException {
        ...
        if (action == Action.POS1 || action == Action.POS2) {
            ...
            cheesecake.getBuilderProcess().build("Fill", composite, origin);
    ...
    public List<String> getLongDesc() {
        return Arrays.asList(
                "The sel command allows you to manipulate Cheesecake's selections, similarly to WorldEdit.",
                "",
