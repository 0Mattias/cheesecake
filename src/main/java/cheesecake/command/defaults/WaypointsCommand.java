...
import cheesecake.api.command.datatypes.ForWaypoints;
...
public class WaypointsCommand extends Command {
    ...
    public WaypointsCommand(ICheesecake cheesecake) {
        super(cheesecake, "waypoints", "waypoint", "wp");
    }
    ...
    public void execute(String label, IArgConsumer args) throws CommandException {
        ...
        if (action == Action.LIST) {
            ...
            IWaypoint[] waypoints = tag != null
                    ? ForWaypoints.getWaypointsByTag(this.cheesecake, tag)
                    : ForWaypoints.getWaypoints(this.cheesecake);
            ...
            IWaypoint[] waypoints = ForWaypoints.getWaypointsByTag(this.cheesecake, tag);
            for (IWaypoint waypoint : waypoints) {
                ForWaypoints.waypoints(this.cheesecake).removeWaypoint(waypoint);
            }
            ...
            List<IWaypoint> deletedWaypoints = this.deletedWaypoints
                    .getOrDefault(cheesecake.getWorldProvider().getCurrentWorld(), Collections.emptyList());
            if (args.peekString().equals("@")) {
                args.get();
                // no args.requireMin(1) because if the user clears an empty tag there is
                // nothing to restore
                while (args.hasAny()) {
            ...
            waypoints.forEach(ForWaypoints.waypoints(this.cheesecake)::addWaypoint);
                ...
                if (action == Action.INFO) {
                    ...
                    recreateComponent.setStyle(recreateComponent.getStyle()
                            .withClickEvent(new ClickEvent.SuggestCommand(
                                    String.format(
                                            "%s%s save %s %s %s %s %s",
                                            Cheesecake.settings().prefix.value, // This uses the normal prefix because
    ...
    public List<String> getLongDesc() {
        return Arrays.asList(
                "The waypoint command allows you to manage Cheesecake's waypoints.",
                "",
