...
import cheesecake.api.utils.NotificationHelper;
import cheesecake.api.utils.SettingsUtil;
import cheesecake.api.utils.TypeUtils;
import cheesecake.api.utils.gui.CheesecakeToast;
...
public final class Settings {
    ...
    /**
     * Disable cheesecake's auto-tool at runtime, but still assume that another mod
     ...
     * Cheesecake itself will not do that.
     */
     ...
     * <p>
     * Decrease to make Cheesecake more often consider paths that would require
     * placing blocks
     */
    ...
    /**
     * Allow Cheesecake to fall arbitrary distances and place a water bucket beneath
     * it.
    ...
    /**
     * Allow Cheesecake to assume it can walk on still water just like any other
     * block.
     * This functionality is assumed to be provided by a separate library that might
     * have imported Cheesecake.
     * <p>
    ...
    /**
     * Blocks that Cheesecake is allowed to place (as throwaway, for sneak bridging,
     * pillaring, etc.)
     */
    ...
    /**
     * Blocks that Cheesecake will attempt to avoid (Used in avoidance)
     */
    public final Setting<List<Block>> blocksToAvoid = new Setting<>(new ArrayList<>(List.of(
            Blocks.TRIPWIRE)));

    /**
     * Blocks that Cheesecake is not allowed to break
     */
    ...
    /**
     * blocks that cheesecake shouldn't break, but can if it needs to.
     */
     ...
     * <p>
     * If a schematic asks for a block on this mapping, Cheesecake will place the
    ...
    /**
     * If this setting is true, Cheesecake will never break a block that is adjacent
     ...
     * pathing scenarios. And they can cause Cheesecake to get trapped indefinitely
    ...
    /**
     * allows cheesecake to save bed waypoints when interacting with beds
     */
    public final Setting<Boolean> doBedWaypoints = new Setting<>(true);

    /**
     * allows cheesecake to save death waypoints
     */
    ...
    /**
     * Allow chat based control of Cheesecake. Most likely should be disabled when
     * Cheesecake is imported for use in
     * something else
     */
    ...
    /**
     * Renders X/Z type Goals as a beacon beam instead of the full-height goal box. Like the box, the
     * beam ignores depth when {@link #renderGoalIgnoreDepth} is on.
     */
    ...
    /**
     * Will cause some minor behavioral differences to ensure that Cheesecake works
     * on anticheats.
     * <p>
    ...
    /**
     * Whether or not to allow you to run Cheesecake commands with the prefix
     */
    ...
    /**
     * Use a short Cheesecake prefix [B] instead of [Cheesecake] when logging to
     * chat
     */
    public final Setting<Boolean> shortCheesecakePrefix = new Setting<>(false);
    ...
    /**
     * This will only allow cheesecake to mine exposed ores, can be used to stop ore
    ...
    /**
     * Build in map art mode, which makes cheesecake only care about the top block
     * in each column
     */
     ...
     * <p>
     * For example, if Cheesecake is doing "mine iron_ore", the instant it breaks
     * the ore (and it becomes air), that location
     ...
     * Cheesecake is extremely good at correcting
     * itself and its paths as it learns new information, as new chunks load. There
     * is no scenario in which having an
     * incorrect cache can cause Cheesecake to get stuck, take damage, or perform
     * any action it wouldn't otherwise, everything
     ...
     * disabled and Cheesecake starts anew
    ...
    /**
     * The function that is called when Cheesecake will log to chat. This function
     * can be added to
    ...
    /**
     * The function that is called when Cheesecake will send a desktop notification.
    ...
    /**
     * The function that is called when Cheesecake will show a toast. This function
     * can be added to
     * via {@link Consumer#andThen(Consumer)} or it can completely be overriden via
     * setting
     * {@link Setting#value};
    ...
    /**
     * Allow the pathfinder to access the cheesecake cache to improve pathing
     */
