...
public class PathingControlManager implements IPathingControlManager {
    ...
    private final HashSet<ICheesecakeProcess> processes; // unGh
    private final List<ICheesecakeProcess> active;
    private ICheesecakeProcess inControlLastTick;
    private ICheesecakeProcess inControlThisTick;
    private PathingCommand command;

    public PathingControlManager(Cheesecake cheesecake) {
        this.cheesecake = cheesecake;
        this.processes = new HashSet<>();
        this.active = new ArrayList<>();
        cheesecake.getGameEventHandler().registerEventListener(new AbstractGameEventListener() { // needs to be after all behavior ticks
            @Override
    ...
    @Override
    public void registerProcess(ICheesecakeProcess process) {
    ...
    public void cancelEverything() { // called by PathingBehavior on TickEvent Type OUT
        ...
        command = null;
        active.clear();
        for (ICheesecakeProcess proc : processes) {
            proc.onLostControl();
    ...
    @Override
    public Optional<ICheesecakeProcess> mostRecentInControl() {
    ...
    public void preTick() {
        ...
        inControlThisTick = null;
        PathingBehavior p = cheesecake.getPathingBehavior();
    ...
    private void postTick() {
        ...
        switch (command.commandType) {
            case FORCE_REVALIDATE_GOAL_AND_PATH:
                ...
                break;
            case REVALIDATE_GOAL_AND_PATH:
                if (Cheesecake.settings().cancelOnGoalInvalidation.value && (command.goal == null || revalidateGoal(command.goal))) {
                    p.softCancelIfSafe();
                }
                p.secretInternalSetGoalAndPath(command);
                break;
    ...
    public boolean forceRevalidate(Goal newGoal) {
        PathExecutor current = cheesecake.getPathingBehavior().getCurrent();
        if (current != null) {
    ...
    public PathingCommand executeProcesses() {
        for (ICheesecakeProcess process : processes) {
            if (process.isActive()) {
        ...
        active.sort(Comparator.comparingDouble(ICheesecakeProcess::priority).reversed());

        Iterator<ICheesecakeProcess> iterator = active.iterator();
        while (iterator.hasNext()) {
            ICheesecakeProcess proc = iterator.next();

            PathingCommand exec = proc.onTick(Objects.equals(proc, inControlLastTick) && cheesecake.getPathingBehavior().calcFailedLastTick(), cheesecake.getPathingBehavior().isSafeToCancel());
            if (exec == null) {
                if (proc.isActive()) {
                ...
                if (!proc.isTemporary()) {
                    iterator.forEachRemaining(ICheesecakeProcess::onLostControl);
                }
                return exec;
            }
        }
