...
public class ToolSet {
    ...
    public ToolSet(ClientPlayerEntity player) {
        ...
        if (Cheesecake.settings().considerPotionEffects.value) {
    ...
    public int getBestSlot(Block b, boolean preferSilkTouch, boolean pathingCalculation) {
         ...
         */
        if (!Cheesecake.settings().autoTool.value && pathingCalculation) {
    ...
    private double avoidanceMultiplier(Block b) {
        return Cheesecake.settings().blocksToAvoidBreaking.value.contains(b)
                ? Cheesecake.settings().avoidBreakingMultiplier.value
                : 1;
    }
