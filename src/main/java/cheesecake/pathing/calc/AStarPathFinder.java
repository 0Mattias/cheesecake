...
package cheesecake.pathing.calc;
...
import cheesecake.pathing.calc.openset.BinaryHeapOpenSet;
import cheesecake.pathing.movement.CalculationContext;
import cheesecake.pathing.movement.Moves;
...
public final class AStarPathFinder extends AbstractNodeCostSearch {
    ...
    protected Optional<IPath> calculate0(long primaryTimeout, long failureTimeout) {
        ...
        boolean slowPath = Cheesecake.settings().slowPath.value;
        if (slowPath) {
            logDebug("slowPath is on, path timeout will be " + Cheesecake.settings().slowPathTimeoutMS.value + "ms instead of " + primaryTimeout + "ms");
        }
        long primaryTimeoutTime = startTime + (slowPath ? Cheesecake.settings().slowPathTimeoutMS.value : primaryTimeout);
        long failureTimeoutTime = startTime + (slowPath ? Cheesecake.settings().slowPathTimeoutMS.value : failureTimeout);
        ...
        int pathingMaxChunkBorderFetch = Cheesecake.settings().pathingMaxChunkBorderFetch.value; // grab all settings beforehand so that changing settings during pathing doesn't cause a crash or unpredictable behavior
        double minimumImprovement = Cheesecake.settings().minimumImprovementRepropagation.value ? MIN_IMPROVEMENT : 0;
        ...
        while (!openSet.isEmpty() && numEmptyChunk < pathingMaxChunkBorderFetch && !cancelRequested) {
            ...
            if (slowPath) {
                try {
                    Thread.sleep(Cheesecake.settings().slowPathTimeDelayMS.value);
