...
import cheesecake.pathing.calc.AbstractNodeCostSearch;
...
import cheesecake.pathing.movement.movements.*;
...
import static cheesecake.api.pathing.movement.MovementStatus.*;
...
public class PathExecutor implements IPathExecutor, Helper {
    ...
    public boolean onTick() {
        ...
        if (pathPosition < path.movements().size() - 1) {
            ...
            if (!behavior.cheesecake.bsi.worldContainsLoadedChunk(next.getDest().x, next.getDest().z)) {
        ...
        if (costEstimateIndex == null || costEstimateIndex != pathPosition) {
            ...
            for (int i = 1; i < Cheesecake.settings().costVerificationLookahead.value && pathPosition + i < path.length() - 1; i++) {
        ...
        if (!movement.calculatedWhileLoaded() && currentCost - currentMovementOriginalCostEstimate > Cheesecake.settings().maxCostIncrease.value && canCancel) {
        ...
        if (movementStatus == SUCCESS) {
            ...
            ticksOnCurrent++;
            if (ticksOnCurrent > currentMovementOriginalCostEstimate + Cheesecake.settings().movementTimeoutTicks.value) {
    ...
    private boolean shouldSprintNextTick() {
        boolean requested = behavior.cheesecake.getInputOverrideHandler().isInputForcedDown(Input.SPRINT);

        // we'll take it from here, no need for minecraft to see we're holding down control and sprint for us
        behavior.cheesecake.getInputOverrideHandler().setInputForceState(Input.SPRINT, false);

        // first and foremost, if allowSprint is off, or if we don't have enough hunger, don't try and sprint
        if (!new CalculationContext(behavior.cheesecake, false).canSprint) {
            return false;
        }
        ...
        if (current instanceof MovementTraverse && pathPosition < path.length() - 3) {
            ...
            if (next instanceof MovementAscend && sprintableAscend(ctx, (MovementTraverse) current, (MovementAscend) next, path.movements().get(pathPosition + 2))) {
                if (skipNow(ctx, current)) {
                    ...
                    onChangeInPathPosition();
                    onTick();
                    behavior.cheesecake.getInputOverrideHandler().setInputForceState(Input.JUMP, true);
                    return true;
                } else {
            ...
            if (pathPosition < path.length() - 2) {
                ...
                if (MovementHelper.canUseFrostWalker(ctx, next.getDest().down())) {
                    ...
                    if (next instanceof MovementTraverse || next instanceof MovementParkour) {
                        boolean couldPlaceInstead = Cheesecake.settings().allowPlace.value && behavior.cheesecake.getInventoryBehavior().hasGenericThrowaway() && next instanceof MovementParkour; // traverse doesn't react fast enough
        ...
        if (current instanceof MovementAscend && pathPosition != 0) {
            ...
            if (prev instanceof MovementDescend && prev.getDirection().up().equals(current.getDirection().down())) {
                ...
                if (ctx.player().getY() >= center.getY() - 0.07) {
                    behavior.cheesecake.getInputOverrideHandler().setInputForceState(Input.JUMP, false);
                    return true;
                }
            }
        ...
        if (current instanceof MovementFall) {
            ...
            if (data != null) {
                ...
                clearKeys();
                behavior.cheesecake.getLookBehavior().updateTarget(RotationUtils.calcRotationFromVec3d(ctx.playerHead(), data.getLeft(), ctx.playerRotations()), false);
                behavior.cheesecake.getInputOverrideHandler().setInputForceState(Input.MOVE_FORWARD, true);
                return true;
            }
        }
        return false;
    ...
    private static boolean sprintableAscend(IPlayerContext ctx, MovementTraverse current, MovementAscend next, IMovement nextnext) {
        if (!Cheesecake.settings().sprintAscends.value) {
            return false;
        }
    ...
    private static boolean canSprintFromDescendInto(IPlayerContext ctx, IMovement current, IMovement next) {
        ...
        return next instanceof MovementDiagonal && Cheesecake.settings().allowOvershootDiagonalDescend.value;
    }
    ...
    private void clearKeys() {
        ...
        behavior.cheesecake.getInputOverrideHandler().clearAllKeys();
    }

    private void cancel() {
        clearKeys();
        behavior.cheesecake.getInputOverrideHandler().getBlockBreakHelper().stopBreakingBlock();
    ...
    private PathExecutor cutIfTooLong() {
        if (pathPosition > Cheesecake.settings().maxPathHistoryLength.value) {
            int cutoffAmt = Cheesecake.settings().pathHistoryCutoffAmount.value;
