...
public class MovementParkour extends Movement {
    ...
    private MovementParkour(ICheesecake cheesecake, BetterBlockPos src, int dist, Direction dir, boolean ascend) {
        super(cheesecake, src, src.relative(dir, dist).above(ascend ? 1 : 0), EMPTY, src.relative(dir, dist).below(ascend ? 0 : 1));
        this.direction = dir;
        this.dist = dist;
        this.ascend = ascend;
    }
    ...
    public MovementState updateState(MovementState state) {
        ...
        if (Cheesecake.settings().allowWalkOnMagmaBlocks.value && ctx.world().getBlockState(ctx.playerFeet().below()).is(Blocks.MAGMA_BLOCK)) {
        ...
        if (ctx.playerFeet().equals(dest)) {
            ...
            if (ctx.playerFeet().equals(src.relative(direction)) || ctx.player().getY() - src.y > 0.0001) {
                if (Cheesecake.settings().allowPlace.value // see PR #3775
                        ...
                        && !ctx.player().onGround()
                        && MovementHelper.attemptToPlaceABlock(state, cheesecake, dest.below(), true, false) == PlaceResult.READY_TO_PLACE
                ) {
