...
public class MovementParkour extends Movement {
    ...
    private MovementParkour(ICheesecake cheesecake, BetterBlockPos src, int dist, Direction dir, boolean ascend) {
        super(cheesecake, src, src.offset(dir, dist).up(ascend ? 1 : 0), EMPTY, src.offset(dir, dist).down(ascend ? 0 : 1));
        this.direction = dir;
        this.dist = dist;
        this.ascend = ascend;
    }
    ...
    public static MovementParkour cost(CalculationContext context, BetterBlockPos src, Direction direction) {
        ...
        return new MovementParkour(context.getCheesecake(), src, dist, direction, res.y > src.y);
    }
    ...
    public static void cost(CalculationContext context, int x, int y, int z, Direction dir, MutableMoveResult res) {
        ...
        if (!context.allowJumpAtBuildLimit && y >= context.world.getTopYInclusive()) {
            return;
        }
    ...
    public MovementState updateState(MovementState state) {
        ...
        if (Cheesecake.settings().allowWalkOnMagmaBlocks.value && ctx.world().getBlockState(ctx.playerFeet().down()).isOf(Blocks.MAGMA_BLOCK)) {
        ...
        if (ctx.playerFeet().equals(dest)) {
            ...
            if (ctx.playerFeet().equals(src.offset(direction)) || ctx.player().getY() - src.y > 0.0001) {
                if (Cheesecake.settings().allowPlace.value // see PR #3775
                        && ((Cheesecake) cheesecake).getInventoryBehavior().hasGenericThrowaway()
                        && !MovementHelper.canWalkOn(ctx, dest.down())
                        && !ctx.player().isOnGround()
                        && MovementHelper.attemptToPlaceABlock(state, cheesecake, dest.down(), true, false) == PlaceResult.READY_TO_PLACE
                ) {
