...
public class MovementPillar extends Movement {
    ...
    public MovementState updateState(MovementState state) {
        ...
        if (ladder) {
            BlockPos against = vine ? getAgainst(new CalculationContext(cheesecake), src) : src.offset(fromDown.get(LadderBlock.FACING).getOpposite());
            if (against == null) {
            ...
            if (thePlayer.getPosition0().getX() != from.getX() || thePlayer.getPosition0().getZ() != from.getZ()) {
                Cheesecake.moveTowardsBlock(from);
            }
             */
            ...
            if (!((Cheesecake) cheesecake).getInventoryBehavior().selectThrowawayForLocation(true, src.x, src.y, src.z)) {
                return state.setStatus(MovementStatus.UNREACHABLE);
            }
