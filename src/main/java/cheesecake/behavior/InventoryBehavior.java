...
public final class InventoryBehavior extends Behavior implements Helper {
    ...
    public InventoryBehavior(Cheesecake cheesecake) {
        super(cheesecake);
    }
    ...
    private boolean requestSwapWithHotBar(int inInventory, int inHotbar) {
        ...
        if (ticksSinceLastInventoryMove < Cheesecake.settings().ticksBetweenInventoryMoves.value) {
            logDebug("Inventory move requested but delaying " + ticksSinceLastInventoryMove + " "
                    + Cheesecake.settings().ticksBetweenInventoryMoves.value);
            return false;
        }
        if (Cheesecake.settings().inventoryMoveOnlyIfStationary.value
                && !cheesecake.getInventoryPauserProcess().stationaryForInventoryMove()) {
    ...
    public boolean hasGenericThrowaway() {
        for (Item item : Cheesecake.settings().acceptableThrowawayItems.value) {
    ...
    public boolean selectThrowawayForLocation(boolean select, int x, int y, int z) {
        ...
        if (maybe != null && throwaway(select, stack -> stack.getItem() instanceof BlockItem
    ...
    public boolean throwaway(boolean select, Predicate<? super ItemStack> desired) {
        return throwaway(select, desired, Cheesecake.settings().allowInventory.value);
    }
