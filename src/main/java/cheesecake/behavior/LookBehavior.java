...
import cheesecake.api.behavior.ILookBehavior;
...
import cheesecake.behavior.look.ForkableRandom;
...
public final class LookBehavior extends Behavior implements ILookBehavior {
    ...
    public LookBehavior(Cheesecake cheesecake) {
        super(cheesecake);
        this.processor = new AimProcessor(cheesecake.getPlayerContext());
        ...
        switch (event.getState()) {
            ...
            case POST: {
                ...
                if (this.prevRotation != null) {
                    ...
                    while (this.smoothYawBuffer.size() > Cheesecake.settings().smoothLookTicks.value) {
                        this.smoothYawBuffer.removeFirst();
                    }
                    this.smoothPitchBuffer.addLast(this.target.rotation.getPitch());
                    while (this.smoothPitchBuffer.size() > Cheesecake.settings().smoothLookTicks.value) {
    ...
    public Optional<Rotation> getEffectiveRotation() {
        if (Cheesecake.settings().freeLook.value) {
    ...
    private static abstract class AbstractAimProcessor implements ITickableAimProcessor {
        ...
        public final void tick() {
            // randomLooking
            this.randomYawOffset = (this.rand.nextDouble() - 0.5) * Cheesecake.settings().randomLooking.value;
            this.randomPitchOffset = (this.rand.nextDouble() - 0.5) * Cheesecake.settings().randomLooking.value;
            ...
            this.randomYawOffset += random * Cheesecake.settings().randomLooking113.value;
        }
        ...
        enum Mode {
            ...
            static Mode resolve(IPlayerContext ctx, boolean blockInteract) {
                ...
                if (ctx.player().isGliding()) {
                    ...
                    // rotation needs to be set somehow, otherwise Cheesecake will halt since objectMouseOver() will just be
