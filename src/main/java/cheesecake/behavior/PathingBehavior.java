...
import cheesecake.api.utils.PathCalculationResult;
import cheesecake.api.utils.interfaces.IGoalRenderPos;
import cheesecake.pathing.calc.AStarPathFinder;
...
import cheesecake.utils.pathing.Favoring;
...
public final class PathingBehavior extends Behavior implements IPathingBehavior, Helper {
    ...
    public PathingBehavior(Cheesecake cheesecake) {
        super(cheesecake);
    }
    ...
    private void dispatchEvents() {
        ...
        for (PathEvent event : curr) {
            cheesecake.getGameEventHandler().onPathEvent(event);
        }
    }
    ...
    public void onTick(TickEvent event) {
        ...
        if (event.getType() == TickEvent.Type.OUT) {
            ...
            cheesecake.getPathingControlManager().cancelEverything();
            return;
        }

        expectedSegmentStart = pathStart();
        cheesecake.getPathingControlManager().preTick();
        tickPath();
        ticksElapsedSoFar++;
        dispatchEvents();
    }
    ...
    private void tickPath() {
        ...
        synchronized (pathPlanLock) {
            ...
            if (Cheesecake.settings().splicePath.value) {
            ...
            synchronized (pathCalcLock) {
                ...
                if (ticksRemainingInSegment(false).get() < Cheesecake.settings().planningTickLookahead.value) {
    ...
    public boolean secretInternalSetGoalAndPath(PathingCommand command) {
        ...
        if (command instanceof PathingCommandContext) {
            ...
            context = new CalculationContext(cheesecake, true);
        }
        if (goal == null) {
            return false;
        }
    ...
    public boolean isSafeToCancel() {
        if (current == null) {
            return !cheesecake.getElytraProcess().isActive() || cheesecake.getElytraProcess().isSafeToCancel();
        }
        return safeToCancel;
    }
    ...
    public boolean cancelEverything() {
        ...
        cheesecake.getPathingControlManager().cancelEverything(); // regardless of if we can stop the current segment, we can still stop the processes
        return doIt;
    }
    ...
    private void findPathInNewThread(final BlockPos start, final boolean talkAboutIt, CalculationContext context) {
        ...
        long failureTimeout;
        if (current == null) {
            primaryTimeout = Cheesecake.settings().primaryTimeoutMS.value;
            failureTimeout = Cheesecake.settings().failureTimeoutMS.value;
        } else {
            primaryTimeout = Cheesecake.settings().planAheadPrimaryTimeoutMS.value;
            failureTimeout = Cheesecake.settings().planAheadFailureTimeoutMS.value;
        }
    ...
    private AbstractNodeCostSearch createPathfinder(BlockPos start, Goal goal, IPath previous, CalculationContext context) {
        ...
        if (Cheesecake.settings().simplifyUnloadedYCoord.value && goal instanceof IGoalRenderPos) {
            BlockPos pos = ((IGoalRenderPos) goal).getGoalPos();
            if (!context.bsi.worldContainsLoadedChunk(pos.getX(), pos.getZ())) {
                transformed = new GoalXZ(pos.getX(), pos.getZ());
            }
        }
        Favoring favoring = new Favoring(context.getCheesecake().getPlayerContext(), previous, context);
        BetterBlockPos feet = ctx.playerFeet();
